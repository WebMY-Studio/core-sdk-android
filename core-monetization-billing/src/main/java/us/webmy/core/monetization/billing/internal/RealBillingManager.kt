...
internal class RealBillingManager(
    ...
    private val fbEvents = FacebookEventsLogger(application)
    ...
    private suspend fun fetchProducts(): Result<Unit> = runCatching {
        ...
        fbEvents.checkTrialConversions(subscriptionPurchases)
        val oneTimeList =
    ...
    private fun handlePurchase(purchase: Purchase) {
        ...
        // Only fresh purchases reach this point unacknowledged; restored ones are
        // already acknowledged and must not be re-logged to Meta
        if (!purchase.isAcknowledged) {
            val details = purchase.products.firstNotNullOfOrNull {
                oneTimeDetails[it] ?: subscriptionDetails[it]
            }
            fbEvents.logNewPurchase(purchase, details)
        }
